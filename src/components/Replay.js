...
const Replay = () => {
  ...
  const setGrid = (info )=>{
    ...
    for (let ship in info.game.player_1_ships){
      let index = 0;
      for (let i = 0; i < info.game.player_1_ships[ship].positions.length; i++){
        makePlayer1Grid[info.game.player_1_ships[ship].positions[i][0]][info.game.player_1_ships[ship].positions[i][1]].ship = `${ship}-${index}`
        index++
      }
    }
    for (let ship2 in info.game.player_2_ships){
      let index = 0;
      for (let j = 0; j < info.game.player_2_ships[ship2].positions.length; j++){
        makePlayer2Grid[info.game.player_2_ships[ship2].positions[j][0]][info.game.player_2_ships[ship2].positions[j][1]].ship = `${ship2}-${index}`
        index++
      }
    }
    ...
    // for (let i = 0; i < moves.length; i++){
        if (moves[i].user_id === game.player_1){
          ...
          if (player2GridNew[moves[i].move[0]][moves[i].move[1]].ship === null){
            ...
            let shipName = player2GridNew[moves[i].move[0]][moves[i].move[1]].ship.slice(0, player2GridNew[moves[i].move[0]][moves[i].move[1]].ship.indexOf("-"));
            player2GridNew[moves[i].move[0]][moves[i].move[1]].attacked = true
            player2GridNew[moves[i].move[0]][moves[i].move[1]].hit = true
            
            setPlayer2Ships({...player2ShipsRef.current, [shipName]: {...player2ShipsRef.current[shipName], hits: player2ShipsRef.current[shipName].hits+1 }})
            console.log('hits', player2ShipsRef.current[shipName].hits, )
            if(player2ShipsRef.current[shipName].hits >= player2Ships[shipName].positions.length){
              //sunk
              for (let j = 0; j < player2Ships[shipName].positions.length; j++){
                  player2GridNew[player2Ships[shipName].positions[j][0]][player2Ships[shipName].positions[j][1]].sunk = true;
              }
            }
          ...
          if (player1GridNew[moves[i].move[0]][moves[i].move[1]].ship === null){
            ...
            let shipName = player1GridNew[moves[i].move[0]][moves[i].move[1]].ship.slice(0, player1GridNew[moves[i].move[0]][moves[i].move[1]].ship.indexOf("-"));
            player1GridNew[moves[i].move[0]][moves[i].move[1]].attacked = true
            player1GridNew[moves[i].move[0]][moves[i].move[1]].hit = true
            setPlayer1Ships({...player1ShipsRef.current, [shipName]: {...player1ShipsRef.current[shipName], hits: player1ShipsRef.current[shipName].hits+1 }})

            if(player1ShipsRef.current[shipName].hits >= player2Ships[shipName].positions.length){
              //sunk
              for (let j = 0; j < player1Ships[shipName].positions.length; j++){
                  player1GridNew[player1Ships[shipName].positions[j][0]][player1Ships[shipName].positions[j][1]].sunk = true;
              }
            }
  ...
  return <div>Replay
    <button onClick={startReplayTimer}>Start Replay</button>
